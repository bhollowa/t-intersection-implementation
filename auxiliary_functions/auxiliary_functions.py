...
initial_positions = [
    (435, 760, 0, 0), (760, 345, 90, 1), (345, 10, 180, 2), (10, 435, 270, 3)
]  # initial positions
...
def reposition_car_in_screen(screen, car):
    """
    Function to graphically reposition a car in the screen so it doesn't get
    lost.
...
def check_close_application(user_input):
    """
    Check if the application must be close depending on the user input (for the
    graphical environment).
    Pressing the x of a pygame window or the escape key will close the window
...
def continue_simulation(user_input):
    ...
    for event in user_input:
        if (event.type == pygame.KEYUP and
                event.key is not pygame.K_1 and
                event.key is not pygame.K_2):
            return True
    return False
...
def coordinator_fail(user_input):
    """
    Check the keyboard user input to make the coordinator to stop answering.
    :param user_input: user inputs of pygame
    :return: <boolean>
    """
    for event in user_input:
        if event.type == pygame.KEYUP and event.key is pygame.K_1:
            return True
    return False


def coordinator_lies(user_input):
    """
    Check the keyboard user input to make the coordinator to stop answering.
    :param user_input: user inputs of pygame
    :return: <boolean>
    """
    for event in user_input:
        if event.type == pygame.KEYUP and event.key is pygame.K_2:
            return True
    return False
...
def random_car(
        ...
        **kwargs):
    """
    Generates a random car with the given name. The max speed is used to give a
    speed not giver than the maximum a the car. the lane can be passed in
    kwargs value if the lane wants to be specified. Example:
    "random_car(4,20,lane=3)"
    ...
    :param kwargs: the lane can be passed in this argument in the "lane"
        argument.
    :param fix: <boolean> boolean for the car to use the fix for the
        supervisory level or not.
    :param stand_still_param: <int> number of times the lenght of the car is
        used for the stand still distance
    :return: a Car object.
    """
    if "lane" in kwargs:
        ...
        if number_of_lanes == 4:
            ...
            if lane == 0:
                ...
                if random_intention == 1:
                    intention = "l"
        intention = "l"
    return Car(name, pos_x, pos_y, direction=direction, lane=lane,
               absolute_speed=initial_speed, intention=intention,
               creation_time=creation_time,
               stand_still_param=stand_still_param, fix=fix)
...
def colliding_cars(car_list):
    ...
    :return: tuple with the colliding cars and True, if the cars collided. None
        and false otherwise.
    """
...
def display_info_on_car(car, display, letter, normalized=1):
    """
    Displays some information of a car on top of it. The Car, display to draw
    on and the font to write. In args params can be passed the "name", "speed"
    and "following" param depending of which want to be displayed.
    The parameter "name" will be displayed at the center, speed to the left and
    following car to the right.
    ...
    :param args: optional arguments to decide which information will be
        displayed.
    :param normalized: reduce the number of the names of the cars so they start
        at 1.
    """
    x, y = car.get_position()
    display.blit(
        letter.render(
            str(car.get_name() - normalized + 1), True, black), (x, y - 30)
    )
    display.blit(
        letter.render(
            str(
...
def create_car_from_json(json_car):
    ...
    left_time = (
        json_car["left_intersection_time"]
        if "left_intersection_time" in json_car else None
    )
    car = Car(json_car["car_name"], pos_x, pos_y, direction=direction,
              absolute_speed=json_car["speed"], lane=lane,
              creation_time=json_car["creation_time"],
              left_intersection_time=left_time)
    return car
...
def create_logs(log_name):
    ...
    setup_logger(
        ...
        "{}collisions{}.log".format(logger_directory, log_name)
    )
    setup_logger(
        "numbers_of_cars{}".format(log_name),
        "{}total_cars{}.log".format(logger_directory, log_name)
    )
    setup_logger(
...
def separate_new_and_old_cars(car_list):
    """
    Function to separate old from new cars. Return a tuple with 2 lists: one
    with new cars, one with old cars ordered by creation time.
    :param car_list: <list> cars at the intersection.
    :return: (<car_list>,<car_list>) Tuple of size 2 with two list: one with
        new cars, the other with old cars ordered by creation time.
    """
    new_cars = []
    old_cars = []
    for car in car_list:
...
def supervisor(car_list):
    ...
    :return: <boolean> True if there is a car with active supervisor level.
        False otherwise.
    """
    for car in car_list:
...
def supervisor_message(messages):
    """
    Check if there is a SupervisorLeftIntersectionMessage in the list of
    messages.
    :param messages: list of messages.
    :return: <boolean>
    """
    # TODO: maybe, MAYBE use oop and implement an is_supervisor_message
    for message in messages:
        supervisor_left_intersection_message = (
            message.__class__.__name__ == "SupervisorLeftIntersectionMessage"
        )
        new_supervisor_message = (
            message.__class__.__name__ == "NewSupervisorMessage"
        )
        if supervisor_left_intersection_message or new_supervisor_message:
            return True
    return False
...
def show_caravan(cars, screen, letter, collided_cars, screen_width,
    ...
    :param collided_cars: information of the collided cars. None if there is no
        collision present.
    ...
    liar_leader_car_surface = pygame.Surface(size)
    liar_leader_car_surface.fill(palette[7])

    for car in cars:
        car_names = [cars[k].get_name() for k in range(len(cars))]
        if not car.get_following_car_name() in car_names:
            leaders.append(car)
        else:
    ...
    for i in range(len(leaders)):
        virtual_cars.append(
            (
                leaders[i],
                pygame.Rect(
                    (screen_width - 100, 700 * (i + 1) / (len(leaders) + 1)),
                    size
                )
            )
        )
    ...
    not_leaders.sort(
        key=lambda not_leader_car: not_leader_car.get_caravan_depth()
    )
    ...
    for car in virtual_cars:
        ...
        if collided_cars is not None:
            first_car_collided = (
                car[0].get_name() == collided_cars[0].get_name()
            )
            second_car_collided = (
                car[0].get_name() == collided_cars[1].get_name()
            )
            if first_car_collided or second_car_collided:
            ...
            if car[0].is_supervisor:
                ...
                elif car[0].supervisor_lies:
                    screen.blit(liar_leader_car_surface, car[1])
                else:
                    screen.blit(leader_car_surface, car[1])
            elif car[0].is_second_at_charge:
                screen.blit(second_at_charge_surface, car[1])
        ...
        screen.blit(
            letter.render(
                str(car[0].get_name() - normalized + 1), True, black
            ),
            car[1].topleft
        )
        screen.blit(
            letter.render(str(car[0].get_caravan_depth()), True, black),
            (car[1].topleft[0], car[1].topleft[1] - 30)
        )
        screen.blit(
            letter.render(
                str(car[0].get_intention() + " " + str(car[0].get_lane())),
                True,
                black
            ),
            car[1].bottomleft
...
def init_graphic_environment(screen_width, screen_height):
    ...
    images_directory = "{}/../images/".format(
        os.path.dirname(os.path.abspath(__file__))
    )
    intersection_background = (
        pygame.image.load("{}background.jpg".format(images_directory))
    )
